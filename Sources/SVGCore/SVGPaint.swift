...
/// Which document's `paintServers` table resolves a `url(#id)` reference.
public enum SVGPaintServerScope: Equatable, Sendable {
    /// The referencing SVG document (including `<use>`-specified paint).
    case document
    /// An externally referenced SVG file (`<use href="other.svg#id">`).
    case external(sourceKey: String)
}
...
public enum SVGPaint: Equatable, Sendable {
    ...
    case color(SVGColor)
    case currentColor
    case paintServer(id: String, fallback: SVGColor? = nil, scope: SVGPaintServerScope = .document)
